...
4. **Deux modules cliniques indépendants du reste** : `Consultation.examens_complementaires[]` et `Consultation.prescriptions[]` sont des sous-documents **texte libre**, jamais des références vers `LabResult`/`ImagingResult`/`Prescription` — la consultation ne « sait » pas quels documents réels ont été générés à partir d'elle. `Prescription.consultation` fait le lien dans l'autre sens pour toute ordonnance. Depuis Correction 4 (6 sept. 2026), `LabResult.consultation`/`ImagingResult.consultation` font le même lien, mais **seulement** pour les examens dont le libellé correspond exactement au catalogue curaté (`backend/utils/examLibelleVersCatalogue.js`) — les examens hors catalogue restent sans lien. Voir `workflows.md` pour le détail du parcours réel. Le reste (couverture complète, migration de la saisie libre vers une sélection catalogue) reste reporté à la **Phase 6** du plan directeur.
