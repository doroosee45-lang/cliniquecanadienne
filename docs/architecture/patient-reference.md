...
**Correction (Correction 6, DATA-004, 6 sept. 2026)** : cette section affirmait auparavant que `ImagingResult` faisait partie des modèles à `patient` requis — vérification fraîche du schéma actuel (`backend/models/ImagingResult.js:5`) infirme cette affirmation : le champ n'y porte aucun `required: true`, contrairement à `LabResult` (son équivalent laboratoire), sans raison métier trouvée à cette asymétrie — c'est une incohérence de schéma réelle, pas un choix documenté, à corriger lors d'une prochaine revue du modèle (hors périmètre de la présente correction, purement documentaire).

Seuls `Appointment`, `Consultation`, `Prescription`, `LabResult`, `Hospitalization`, `DossierChirurgical` et **`Echographie`** déclarent `patient` **requis** (`required: true` revérifié dans chacun de ces 7 schémas le 6 sept. 2026) — les modules où un dossier `Patient` structuré est une précondition métier réelle (on ne prend pas un RDV, ne consulte pas, ne prescrit pas, n'hospitalise pas, n'ouvre pas un dossier chirurgical, ne réalise pas une échographie pour un patient totalement anonyme). `ImagingResult` reste optionnel comme les autres modèles de cette section (§25 texte libre de repli), au même titre que ses pairs, malgré son rôle métier proche de `LabResult`.

**Exception réelle vérifiée (Correction 6, 6 sept. 2026)** : `required: true` sur `Echographie.patient` n'est appliqué par Mongoose qu'à la validation au moment de la sauvegarde — un document déjà en base et antérieur à cette contrainte n'est jamais revalidé rétroactivement. Un tel document orphelin existe réellement dans la base (`_id 6a27e3735c7cc215169cc97d`, `patient_nom: "meya"`, `dossier: "151515"`, créé le 2026-06-09, aucun champ `patient` du tout). Recherche de rattachement menée sur les 12 patients réels de la base (nom, prénom, numéro de dossier, sexe, âge calculé à la date de l'examen) : `dossier "151515"` ne correspond au format d'aucun numéro de dossier réel (`CLIN-2026-XXXXX`) ; le candidat le plus proche par nom (« Meye Brigitte », `CLIN-2026-00011`) a un âge calculé de 34 ans au 2026-06-09 contre 55 ans saisis sur le document — écart trop important pour conclure à un rattachement fiable. Décision : document conservé en l'état, orphelin accepté — aucun patient fictif créé pour le rattacher artificiellement.
...
- Pour toute nouvelle relation vers `Patient`, utiliser le nom de champ `patient` (la convention cible, déjà majoritaire — voir §2), et préférer une référence optionnelle avec repli texte libre si le module doit rester utilisable sans dossier structuré (aligné sur l'existant), une référence requise seulement si c'est une vraie précondition métier (comme Appointment/Consultation/Prescription/LabResult/Hospitalization/DossierChirurgical/Echographie — voir §3 pour le cas d'`ImagingResult`, resté optionnel malgré un rôle métier proche de `LabResult`).
