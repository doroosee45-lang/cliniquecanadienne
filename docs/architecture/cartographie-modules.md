...
| 5 | `Consultation` ne référence pas les `Prescription`/`LabResult`/`ImagingResult` réellement créés à partir d'elle (sous-documents texte libre) | **Partiellement résolu par anticipation (Correction 4, 6 sept. 2026)** pour LabResult/ImagingResult sur les examens à correspondance catalogue exacte (`consultation` réellement renseigné) — reste ouvert pour les examens hors catalogue curaté. Reporté à la **Phase 6** du plan directeur pour le reste (voir `workflows.md` §2-3, `relationships.md` constat #4) |
