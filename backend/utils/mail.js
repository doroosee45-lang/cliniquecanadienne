...
// serveur, aucune configuration parallèle via l'UI Settings. Pas de mise en
// cache du client ni de la clé : getClient()/isConfigured() relisent
// env.RESEND_API_KEY à chaque appel (jamais une constante figée au chargement
// du module), pour toujours refléter sa valeur courante — utile en test
// (mailResendHonesty.test.js mute env.RESEND_API_KEY entre sous-tests), sans
// coût réel en production où la clé ne change jamais en cours de vie du
// processus. Une valeur figée au chargement causerait, une fois une vraie clé
// chargée au premier require(), un appel réseau réel à Resend même quand un
// test s'attend explicitement au mode simulé.
const getClient = () => new resendSdk.Resend((env.RESEND_API_KEY || '').trim());

const isConfigured = () => !!(env.RESEND_API_KEY || '').trim();

if (!isConfigured()) {
  logger.warn('[MAIL] RESEND_API_KEY non configurée — emails simulés tant que cette variable n\'est pas définie.');
