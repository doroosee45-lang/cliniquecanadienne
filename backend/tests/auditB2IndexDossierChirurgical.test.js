// AUDIT-B2 — DossierChirurgical.patient_id n'avait aucun index malgré un
// filtrage fréquent par ce champ (chirurgieController.js::getDossiers).
// Ce test prouve que l'index composé {patient_id:1, created_at:-1} est
...
test('B2 — index composé patient_id/created_at construit sur DossierChirurgical (base réelle)', { skip: !process.env.MONGO_URI && 'MONGO_URI non configuré' }, async (t) => {
  await mongoose.connect(process.env.MONGO_URI);
  const DossierChirurgical = require('../models/DossierChirurgical');

  try {
    await t.test('l\'index {patient_id:1, created_at:-1} existe réellement sur la collection', async () => {
      await DossierChirurgical.syncIndexes();
      const indexes = await DossierChirurgical.collection.indexes();
      const found = indexes.find(i => i.key && i.key.patient_id === 1 && i.key.created_at === -1);
