// MODULE AI — sous-module Imagerie IA : preuve RBAC réelle (HTTP) pour
// GET /ai/imaging-insights/:patientId. Un rôle non listé dans
// ai.routes.js ne doit jamais pouvoir appeler cet endpoint directement,
// même en connaissant l'URL — vérifié via une vraie requête HTTP contre
// un vrai serveur (mongod local isolé), pas un test interne au
// contrôleur qui contournerait le middleware réel.
...
const PASSWORD = 'AiImgRbacTestPass1!';
...
test('AI/Imagerie — GET /ai/imaging-insights/:patientId réellement protégé par rôle (serveur isolé, vraies requêtes HTTP)', { skip: !mongodExists() && 'mongod introuvable — infrastructure de serveur isolé indisponible' }, async (t) => {
  let server = null;
  let connected = false;
  ...
  try {
    ...
    const medecin = await User.create({ email: `_aiimg-medecin-${stamp}@_test.local`, password: PASSWORD, nom: 'T', prenom: 'Medecin', role: 'medecin', statut: 'actif' });
    created.users.push(medecin._id);
    const pharmacien = await User.create({ email: `_aiimg-pharma-${stamp}@_test.local`, password: PASSWORD, nom: 'T', prenom: 'Pharma', role: 'pharmacien', statut: 'actif' });
    created.users.push(pharmacien._id);
    const patient = await Patient.create({ nom: `AIImgRbac-${stamp}`, prenom: 'Synthetique', sexe: 'M', date_naissance: '1990-01-01' });
    ...
    await t.test('rôle autorisé (medecin) — requête acceptée (200)', async () => {
      ...
      const res = await fetch(`${server.baseUrl}/ai/imaging-insights/${patient._id}`, withTimeout({ headers: { Cookie: cookie } }));
    ...
    await t.test('non authentifié — appel direct sans cookie refusé (401)', async () => {
      const res = await fetch(`${server.baseUrl}/ai/imaging-insights/${patient._id}`, withTimeout({}));
