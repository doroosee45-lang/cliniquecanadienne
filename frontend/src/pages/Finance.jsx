...
function Modal({ open, onClose, title, children, maxWidth = 620 }) {
  ...
  return (
    <div className="fmov" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div ref={boxRef} className="fmov-box" style={{ maxWidth }} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
        <div className="fmov-hdr">
          <h3 id={titleId}>{title}</h3>
          <button className="fmov-cls" onClick={onClose} aria-label="Fermer">×</button>
        </div>
