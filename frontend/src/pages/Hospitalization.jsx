...
const normalizeHosp = h => ({
  ...
  medecin_label:   h.medecin_responsable
    ? `${h.medecin_responsable.prenom || ''} ${h.medecin_responsable.nom || ''}`.trim()
    : (h.medecin_nom || h.medecin || '—'),
  service_label:   h.service?.nom || h.service_nom || (typeof h.service === 'string' ? h.service : '') || '',
  date_admission:  h.date_entree || h.date_admission || '',
  numero:          h.numero || String(h._id || '').slice(-6).toUpperCase(),
});
...
function Modal({ open, onClose, title, children, maxWidth = 720 }) {
  ...
  return (
    <div className="hov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div ref={boxRef} className="hov-box" style={{ maxWidth }} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
        <div className="hov-hdr">
          <h3 id={titleId}>{title}</h3>
          <button className="hov-cls" onClick={onClose} aria-label="Fermer">×</button>
        </div>
...
export default function Hospitalisation() {
  ...
  // ── Source de vérité unique pour les hospitalisations ────
  ...
  // ── Chargement paginé/filtré pour la liste ────────────────
  const loadHosps = useCallback(async () => {
    ...
    try {
      ...
      if (!search && !filterStatut && page === 1) {
        setKpis(computeKpis(list, data.taux_occupation || data.kpis?.taux_occ || 0));
      }
    } catch(err) {
  ...
  const createHosp = async (e) => {
    ...
    // Notification immédiate "en cours"
    ...
    try {
      ...
      setModalAdmission(false);
      setFormHosp(EMPTY_HOSP);

      // 2. Ajouter immédiatement en tête de liste (sans attendre le rechargement)
      ...
      toast.success(
        `✅ Admission ${newHosp.numero || ""} créée avec succès`,
        { id: toastId, duration: 5000 }
      );

      // 5. Rechargement complet en arrière-plan pour synchroniser
      setTimeout(() => {
        loadHosps();
        loadAllForKpis();
  ...
  const updateHosp = async (updates) => {
    ...
    try {
      ...
      // Recalculer les KPIs si le statut a changé
      if (updates.statut && updates.statut !== currentHosp.statut) {
        setHosps(prev => {
          setKpis(computeKpis(prev.map(h => h._id === currentHosp._id ? updated : h)));
          return prev;
        });
      }
  ...
  const addConstante = async (e) => {
    ...
    try {
      ...
      setModalConstante(false);
      setFormConstante(EMPTY_CONSTANTE);
    } catch {
  ...
  const addTraitement = async (e) => {
    ...
    try {
      ...
      setModalTraitement(false);
      setFormTraitement(EMPTY_TRAITEMENT);
    } catch {
      toast.error("❌ Échec de l'ajout", { id: toastId });
    }
  };
              ...
              <div className="ho-card hofu">
                ...
                {loading ? (
                  ...
                  <div style={{ padding:40, textAlign:"center", color:"var(--cm)" }}>
                    ...
                    <div style={{ fontWeight:600, marginBottom:6 }}>Aucune hospitalisation enregistrée</div>
                    <button className="hbtn hbtn-teal" onClick={() => { setFormHosp(EMPTY_HOSP); setModalAdmission(true); }}>{I.plus} Créer la première admission</button>
                  </div>
                ) : (
                        ...
                        {hosps.slice(0,8).map(d => {
                          ...
                          return (
                            <tr key={d._id} style={{ background: d.statut === "attente" ? "#FEF9E7" : d.statut === "observation" ? "#EBF5FB" : "" }}>
                              ...
                              <td>
                                {d.chambre_label ? <Badge cls="blue">{d.chambre_label} · Lit {d.lit_label}</Badge> : <span style={{ color:"var(--cm)", fontSize:11 }}>Non attribué</span>}
                                {d.type_chambre && <div style={{ fontSize:10, color:"var(--cm)", marginTop:3 }}>{TYPE_CHAMBRE[d.type_chambre] || d.type_chambre}</div>}
                              </td>
                              <td style={{ fontSize:12, color:"var(--cm)" }}>{d.service_label || "—"}</td>
                              <td style={{ fontSize:12, color:"var(--cm)" }}>{d.medecin_label || "—"}</td>
                      ...
                      {loading ? (
                        ...
                        return (
                          <tr key={d._id} style={{ background: d.statut === "attente" ? "#FEF9E7" : "" }}>
                            ...
                            <td style={{ fontSize:12, color:"var(--cm)" }}>{d.service_label || "—"}</td>
                            <td>
                              {d.chambre_label ? <><Badge cls="blue">{d.chambre_label} · Lit {d.lit_label}</Badge><div style={{ fontSize:10, color:"var(--cm)", marginTop:2 }}>{TYPE_CHAMBRE[d.type_chambre]}</div></> : <span style={{ color:"var(--cm)", fontSize:12 }}>Non attribué</span>}
                            </td>
                            <td style={{ fontSize:12, color:"var(--cm)" }}>{d.medecin_label || "—"}</td>
            ...
            <div>
              ...
              {lits.length === 0 ? (
                <div className="ho-card" style={{ padding:40, textAlign:"center" }}>
                  <div style={{ fontSize:32, marginBottom:12 }}>🏥</div>
                  <div style={{ color:"var(--cm)", fontSize:13 }}>Aucune chambre configurée dans le système</div>
                </div>
              ) : (
                <>
                  ...
                  {[...new Set(lits.map(l => l.batiment))].filter(Boolean).map((bat, bi) => {
                    const batLits = lits.filter(l => l.batiment === bat);
                    const palette = [
                      { color:"var(--hb)", light:"#EBF5FB" },
                      { color:"var(--ht)", light:"#E8F8F5" },
                      { color:"var(--hg)", light:"#EAFAF1" },
                    ][bi % 3];
                    return (
                      <div key={bat} className="ho-card hofu" style={{ marginBottom:20 }}>
                        <div className="ho-card-hdr" style={{ background:`linear-gradient(to right,${palette.light},transparent)` }}>
                          <h3 style={{ color:palette.color }}>🏥 {bat}</h3>
                          <span style={{ fontSize:12, color:"var(--cm)" }}>{batLits.filter(l=>l.statut==="libre").length} / {batLits.length} lits libres</span>
                        </div>
                        <div style={{ padding:20, display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:14 }}>
                          {batLits.map(l => (
                            <div key={l._id || l.id} className={`lit-card ${l.statut} hofu`}>
                              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                                <div style={{ fontWeight:700, color:"var(--hn)", fontSize:14 }}>Ch. {l.chambre || l.numero} — Lit {l.lit || l.numero_lit}</div>
                        ...
                        <div style={{ padding:"8px 20px 14px" }}>
                          <Prog pct={Math.round(batLits.filter(l=>l.statut==="occupe").length / batLits.length * 100)} color={palette.color} />
                          <div style={{ fontSize:11, color:"var(--cm)", marginTop:4 }}>Taux d'occupation : {Math.round(batLits.filter(l=>l.statut==="occupe").length / batLits.length * 100)}%</div>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                    <div style={{ background:"rgba(255,255,255,.1)", borderRadius:12, padding:"10px 16px", minWidth:160 }}>
                      ...
                      <div style={{ fontSize:13, fontWeight:600, color:"#A2D9CE", marginBottom:2 }}>
                        🏥 {currentHosp.batiment} — {currentHosp.chambre_label} · Lit {currentHosp.lit_label}
                      </div>
                    ...
                    <div className="ho-card">
                      ...
                      <div style={{ padding:20, display:"flex", flexDirection:"column", gap:14 }}>
                        <div style={{ background:"#EAF4FB", border:"1.5px solid #AED6F1", borderRadius:12, padding:16, textAlign:"center" }}>
                          ...
                          <div style={{ fontWeight:800, fontSize:18, color:"var(--hn)" }}>{currentHosp.chambre_label} — Lit {currentHosp.lit_label}</div>
                        ...
                        <div className="ho-g11s">
                          <div>
                            ...
                            <input className="hinp" value={currentHosp.chambre_label || currentHosp.chambre_num || ""} onChange={e => setCurrentHosp(d => ({ ...d, chambre_label:e.target.value, chambre_num:e.target.value }))} placeholder="Ex: A12" />
                          </div>
                          <div>
                ...
                <div style={{ marginTop:20 }}>
                  ...
                  {constantes.length > 0 && (
                    <div className="ho-g5" style={{ marginBottom:20 }}>
                      {[
                        { lbl:"Température",      val: constantes[0].temperature ? `${constantes[0].temperature}°C` : "—", cls: constantes[0].temperature > 38.5 ? "danger" : constantes[0].temperature < 36 ? "warn" : "ok" },
                        { lbl:"Tension",          val: constantes[0].tension_sys && constantes[0].tension_dia ? `${constantes[0].tension_sys}/${constantes[0].tension_dia}` : constantes[0].tension || "—", cls:"" },
                        { lbl:"Fréquence card.",  val: constantes[0].fc ? `${constantes[0].fc} bpm` : "—", cls: constantes[0].fc > 100 || constantes[0].fc < 55 ? "danger" : "ok" },
                        { lbl:"SpO₂",             val: constantes[0].spo2 ? `${constantes[0].spo2}%` : "—", cls: constantes[0].spo2 < 94 ? "danger" : "ok" },
                        { lbl:"Poids",            val: constantes[0].poids ? `${constantes[0].poids} kg` : "—", cls:"" },
                      ].map(v => (
                  ...
                  <div className="ho-card">
                    ...
                    {constantes.length === 0 ? (
                      <div style={{ padding:30, textAlign:"center", color:"var(--cm)", fontSize:13 }}>Aucune constante enregistrée</div>
                    ) : (
                      <div style={{ overflowX:"auto" }}>
                        <table className="ho-tbl">
                          <thead>
                          ...
                          <tbody>
                            {constantes.map((c,i) => (
                              <tr key={c._id || c.id || i}>
                                ...
                                <td><span style={{ fontWeight:600, color: c.temperature > 38.5 ? "var(--hr)" : c.temperature < 36 ? "var(--ho)" : "var(--hg)" }}>{c.temperature || "—"}</span></td>
                                <td style={{ fontSize:12 }}>{c.tension_sys && c.tension_dia ? `${c.tension_sys}/${c.tension_dia}` : c.tension || "—"}</td>
                                <td><span style={{ fontWeight:600, color: c.fc > 100 || c.fc < 55 ? "var(--hr)" : "var(--hg)" }}>{c.fc || "—"}</span></td>
                                <td><span style={{ fontWeight:600, color: c.spo2 < 94 ? "var(--hr)" : "var(--hg)" }}>{c.spo2 || "—"}</span></td>
                                <td style={{ fontSize:12 }}>{c.poids || "—"}</td>
                                <td style={{ fontSize:11, color:"var(--cm)", maxWidth:200 }}>{c.note_med || c.note || "—"}</td>
                  ...
                  <div className="ho-card">
                    ...
                    <div style={{ padding:16 }}>
                      {traitements.length === 0 ? (
                        <div style={{ padding:20, textAlign:"center", color:"var(--cm)", fontSize:13 }}>Aucun traitement enregistré</div>
                      ) : traitements.map((t,i) => (
                        <div key={t._id || t.id || i} className="trait-row">
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ fontSize:11, color:"var(--cm)" }}>{t.dose} · Voie {t.voie} · {t.heure} · {t.personnel}</div>
                          </div>
                  ...
                  <div className="ho-card">
                    ...
                    <div style={{ padding:16 }}>
                      {prescriptions.length === 0 ? (
                        <div style={{ padding:20, textAlign:"center", color:"var(--cm)", fontSize:13 }}>Aucune prescription enregistrée</div>
                        ...
                        return (
                          <div key={type} style={{ marginBottom:16 }}>
                            <div style={{ fontSize:12, fontWeight:700, color:colors[type], marginBottom:8, display:"flex", alignItems:"center", gap:6 }}>{icons[type]} {labels[type]}</div>
                            {items.map((p,i) => (
                              <div key={p._id || p.id || i} style={{ background:"#F4F9FD", border:"1.5px solid var(--cbr)", borderRadius:10, padding:"10px 14px", marginBottom:8 }}>
                  ...
                  {["labo","imagerie"].map(type => {
                    ...
                    return (
                      <div key={type} className="ho-card" style={{ marginBottom:16 }}>
                        ...
                        {items.length === 0 ? (
                          <div style={{ padding:20, textAlign:"center", color:"var(--cm)", fontSize:13 }}>Aucun examen demandé</div>
                        ) : (
                          <div style={{ overflowX:"auto" }}>
                            <table className="ho-tbl">
              ...
              {section === "visites" && (
                <div style={{ marginTop:20 }}>
                  ...
                  <div className="ho-card">
                    ...
                    {visites.length === 0 ? (
                      <div style={{ padding:30, textAlign:"center", color:"var(--cm)", fontSize:13 }}>Aucune visite enregistrée</div>
                    ) : (
                      <div style={{ overflowX:"auto" }}>
                        <table className="ho-tbl">
                          <thead><tr><th>Date</th><th>Visiteur</th><th>Heure entrée</th><th>Heure sortie</th><th>Durée</th><th>Note</th></tr></thead>
                          <tbody>
                            {visites.map((v,i) => {
          ...
          <form onSubmit={createHosp}>
            <div className="ho-g11" style={{ gap:14 }}>
              <div style={{ gridColumn:"1/-1" }}>
                ...
                {patients.length === 0 && (
                  <div style={{ fontSize:11, color:"var(--ho)", marginTop:4 }}>⚠️ Aucun patient chargé — vérifiez la connexion à l'API</div>
                )}
              </div>
              <div>
            ...
            <div style={{ display:"flex", gap:10, marginTop:20 }}>
              ...
              <button type="submit" className="hbtn hbtn-teal" style={{ marginLeft:"auto" }} disabled={saving}>
                {I.save} {saving ? "Création en cours..." : "Créer l'admission"}
              </button>
            </div>
          </form>
        </Modal>
          ...
          <form onSubmit={addConstante}>
            <div className="ho-g11" style={{ gap:14 }}>
              ...
              <div style={{ gridColumn:"1/-1" }}>
                ...
                <textarea className="hinp" rows={2} value={formConstante.note_med} onChange={e => setFormConstante(f=>({...f,note_med:e.target.value}))} placeholder="Notes du médecin..." />
              </div>
              <div style={{ gridColumn:"1/-1" }}>
          ...
          <form onSubmit={addTraitement}>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div>
                ...
                <input className="hinp" required placeholder="Ex: Paracétamol 1g..." value={formTraitement.medicament} onChange={e => setFormTraitement(f=>({...f,medicament:e.target.value}))} />
              </div>
              <div className="ho-g11s">
                <div>
          ...
          <form onSubmit={addExamen}>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <input className="hinp" required placeholder="Ex: NFS, CRP, Radiographie thorax..." value={formExamen.designation} onChange={e => setFormExamen(f=>({...f,designation:e.target.value}))} />
              </div>
              <div>
          ...
          <form onSubmit={async e => {
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <textarea className="hinp" rows={2} placeholder="Ex: 3 fois/jour pendant 7 jours..." value={formPrescription.posologie} onChange={e => setFormPrescription(f=>({...f,posologie:e.target.value}))} />
              </div>
              <div>
